...
def sensores():
    pub = rospy.Publisher('lectura_sensores',String,queue_size=10)
    ...
    while not rospy.is_shutdown():
        ...
        # Conversion de las mediciones a grados
        med_imu=[ax, ay, az, mx, my, mz, gx, gy, gz]
        med_imu_g= [valor*np.pi/180 for valor in med_imu]
        med_imu_g_r=[round(x,2) for x in med_imu_g]

        #Aqui se carga los valores de los encoders
        encoders=[100,200]

        mediciones=ultrasonicos_r+med_imu_g_r+encoders

        #mensaje=' '.join(str(mediciones))
        mensaje = str(mediciones).replace(","," ")
